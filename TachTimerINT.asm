...
IncNumCycles:
   	cmp [_cNumCycles], 0		; check if it's 
   	jnz NotZeroNumCycles
	lcall TachTimer_wReadTimerSaveCV 
   	mov [_wFirstValue], X	;load upper byte with input(capture count)
	mov [_wFirstValue+1], A ;load lower byte
NotZeroNumCycles:
	lcall TachTimer_wReadTimerSaveCV
	mov [_wLastValue], X load last valeue with input(caputre count)
   	mov [_wLastValue+1], A
   ;---------------------------------------------------
